...
print("os_bits =", os_bits)
print("os_arch =", os_arch)
...
__winnt = os_type == "windows"
__posix = not __winnt

def winnt(func):
    return _ifdef_decorator_impl(__winnt, func, sys._getframe().f_back)

def posix(func):
    return _ifdef_decorator_impl(__posix, func, sys._getframe().f_back)

def load_external_shared_library(file):
    '''
    Load an external shared library (Eg. "file_name", "file_name.ext", "path/to/file_name.ext")
    '''
    PLATFORM_SHARED_EXTENSION = { "windows": ".dll", "linux": ".so", "android": ".so", "macos": ".dylib", "Darwin": ".ios" }
    ...
    try: result = CDLL(file)
        ...
        if l[-1] == "":
            file_ext   = PLATFORM_SHARED_EXTENSION.get(os_type)
            file_name += file_ext
